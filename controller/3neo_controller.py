# Modified Controller_HP: Changed 100 <-> 200
...
class SimpleSwitch13(app_manager.RyuApp):
	...
	# Adding default rules in table 200
	def add_table_200_redirection(self, dp, buffer_id):
		ofproto = dp.ofproto
	...
	#software match i.e. tabel 200 for all ICMP packets
	...
	def add_redirection_flows(self, dp, buffer_id,dstip,srcip):
		overt_ip = '192.168.2.4'
		ofproto = dp.ofproto
		parser = dp.ofproto_parser
		
		# FOR TRAFFIC FROM CLIENT TO OD
		# matches in software (Table 200)
	...
	def add_flow(self, datapath, priority, match, actions, buffer_id=None, table_id=200, redirect_table=False, src_ip="", dst_ip=""):
		...
		if redirect_table:
			inst = []
			match1 = parser.OFPMatch(ipv4_src=src_ip, ipv4_dst=dst_ip)
			inst.append(parser.OFPInstructionGotoTable(100))
			msg = parser.OFPFlowMod(datapath=datapath, table_id=200, priority=3, match=match1, instructions=inst) # more priority than the controller, but less than the decoy flows
			datapath.send_msg(msg)
	...
	def _packet_in_handler(self, ev):
		...
		self.add_redirection_flows(datapath, msg.buffer_id,dstip,srcip)
		...
		if self.flag is False:
			...
			self.add_table_200_redirection(datapath, msg.buffer_id)
		...
		if out_port != ofproto.OFPP_FLOOD:
			...
			if msg.buffer_id != ofproto.OFP_NO_BUFFER:
				self.add_flow(datapath, 2, match, actions, msg.buffer_id, table_id=100, redirect_table=False)
				if ipv is not None:
					#print "Adding IP flow for this"
					self.add_flow(datapath, 2, match, actions, msg.buffer_id, table_id=100, redirect_table=True, src_ip = ipv.src_ip, dst_ip=ipv.dst_ip)
			else:
				#print "adding flow2"
				self.add_flow(datapath, 2, match, actions, table_id=100, redirect_table=False)#,  src_ip = ipv.src_ip, dst_ip=ipv.dst_ip)
		if msg.buffer_id == ofproto.OFP_NO_BUFFER:
			data = msg.data
